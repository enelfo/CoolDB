...
<h3 align="center">FastDB - это модуль для удобного использования баз данных на основе sqlite3 - для синхронного и асинхронного взаимодействия с базами данных.</h3>
<h1></h1>


<h4>SyncDataBaseRequests - это надстройка над sqlite3, призваная ускорить работу с базами данных sqlite в синхронном режиме.</h4>

> <h5>Пример синхронного использования</h5>
> Прелесть синхронного режима функций в том, что мы можем вызывать их, как в синхронном режиме, так и в асинхронном из кроутин, либо блокиря их действие, либо прокидывая их в асинхронный режим, по средствам asyncio.to_thread().
> И все таки более предпочтительным будет использовать асинхнонные фарианты функций, чтобы попросту экономить мощностя.
...
<h4>AsyncDataBaseRequests - это надстройка над aiosqlite, призваная ускорить работу с базами данных sqlite в асинхронном режиме.</h4>
...
<h4>Разница менжу синхронном и асинхронным режимами в импортруемых модулях и в том, что в асинхронном режими функции вываются из кроутин с добавлением ключевого слова await.</h4>
<h1></h1>
...
<h6 align="center">Функция get_db() используется для создания таблицы в базе данных, если такой еще не существуюет. Попутно создает и базу данных, если ее еще не существует.</h6>
...
1. dataBase - путь до файла базы данных. Должен передоваться в формате строки формата: "path/filename".
> <h5>Примечания</h5>
> Предпочтительно хранить файл базы данных не глуюже одной поддиректории от main файла

2. table_name - название таблицы в базе данных. Должен передоваться в формате строки.

3. columns - столбцы, с которыми необходимо будет создать базу данных, если она еще не была создана. Должны передоваться в формате строки формата: "(SQL_VARS)".
> <h5>Пример</h5>
...
```
> <h5>Примечания</h5>
> В асинхронном режиме должна вызываться из кроутины и должно присутствовать ключивое слово await.
<h1></h1>
...
<h6 align="center">Функция insert_to_db() используется для добавления элементов во всех столбцы таблицы. Попутно создает и базу данных, если ее еще не существует.</h6>
...
3. parameters - параметры, которые необходимо будет поместить в базу данных. Должены передоваться в формате списка из переменных или значений.
...
```
> <h5>Примечания</h5>
> insert_to_db() записывает значения во все существующие столюцы, поэтому количество передаваемых параметров, должно быть равным количеству существующих столбцов.
> В асинхронном режиме должна вызываться из кроутины и должно присутствовать ключивое слово await.
<h1></h1>
...
4. parameter - параметр, который необходимо будет поместить в базу данных или несколько параметров для определенных столбцов. Должены передоваться в формате строки формата "'X', 'Y', 'N'".
...
<h6 align="center">Функция fetch_all_where() используется для извлечения элементов из всех столбцов всех строк, в которых параметр поиска равет значению параметра поиска - условно, извлечь все столбцы всех строк, где айди=13.</h6>
